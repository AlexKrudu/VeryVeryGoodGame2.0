...
def main():
    ...
    anim = [pygame.transform.scale(player_img.subsurface((12, 31, 56, 124)), (32, 64)),
            pygame.transform.scale(player_img.subsurface((93, 31, 41, 124)), (26, 64)),
            pygame.transform.scale(player_img.subsurface((162, 31, 68, 123)), (32, 64)),
            pygame.transform.scale(player_img.subsurface((252, 31, 61, 125)), (32, 64))]
    ...
    tom_die_anim = [pygame.transform.scale(tom_death_image.subsurface((50, 0, 50, 123)), (32, 64)),
                            ...
                            if map.objects[y][k] in [30, 31, 40,  50, 61, 71, 91] and map.objects[y][k] != map.objects[i][j]:
                                ...
                                if e.pos[0] < player.rect.x:
                                    if e.pos[0] <= map.obj[y][k].Rect.x + map.obj[y][k].Rect.width <= player.rect.x and \
                                                    not map.obj[y][k].state:
                                        flag = True
                                else:
                                    if player.rect.x <= map.obj[y][k].Rect.x <= e.pos[0] and not map.obj[y][k].state:
                                        flag = True
                        if not flag:
            ...
            if e.type == pygame.MOUSEBUTTONDOWN and e.button == 3 and not player.on_ladder:
                ...
                for i in range(len(map.objects[y])):
                    if map.objects[y][i]in [30, 31, 40,  50, 61, 71, 91]:
                        if map.obj[y][i].Rect.collidepoint(e.pos) and not map.obj[y][i].state:
                            flag = True
                        if e.pos[0] < player.rect.x:
                            if e.pos[0] <= map.obj[y][i].Rect.x + map.obj[y][i].Rect.width <= player.rect.x and not map.obj[y][i].state:
                                flag = True
                        else:
                            if player.rect.x <= map.obj[y][i].Rect.x <= e.pos[0] and not map.obj[y][i].state:
                                flag = True
